...
class DistributionCode extends Validate
{
    ...
    protected function checkCode($code, $other, $data)
    {
        ...
        $firstLader = \app\common\model\User::field('id,is_distribution,ancestor_relation')
        ...
        // 判断上级是否为分销会员
        $distribution = \app\common\model\Distribution::where('user_id', $firstLader['id'])->findOrEmpty()->toArray();
        ...
        // 只有分销会员才能邀请粉丝
        if(!in_array(1, $config['qualifications']) && in_array(2, $config['qualifications']) && !$isDistribution) {
            return '非分销会员不允许邀请粉丝';
        }
