...
//                    throw new Exception('商品：'.implode('、',$goods_name).'不支持快递配送！');
    ...
    public static function getOrderList($user_id, $type, $page, $size)
    {
        ...
        $lists = $order->where(['del' => 0, 'user_id' => $user_id])
            ->where($where)
            ->with(['orderGoods'])
            ->field('id,order_sn,order_status,pay_status,order_amount,order_status,order_type,shipping_status,create_time,delivery_type,team_found_id')
            ->page($page, $size)
            ->order('id desc')
            ->select();
            ...
            if ($express && $order['shipping_status']) {
                ...
                if ($app && $key) {
                    ...
                    if ($shipping_code === 'SF' && $express === 'kdniao') {
                        $expressage->logistics($shipping_code, $order_delivery['invoice_no'], substr($order['mobile'],-4));
                    }else {
