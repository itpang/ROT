...
class VerificationLogic
{
    ...
    public static function exportFile($params)
    {
        ...
        $field = 'o.*,o.order_type as order_type_text, o.order_source as order_source_text,o.pay_way as pay_way_text,o.pay_status as pay_status_text,o.order_status as order_status_text,u.sn as user_sn,u.nickname as user_nickname,u.level as user_level,ss.name as shop_name';
        ...
        $exportTitle = ['订单编号', '订单类型', '下单时间', '支付时间', '订单来源', '会员编号', '会员昵称', '会员等级', '商品信息', '商品数量', '运费金额', '商品总金额', '优惠金额', '应付金额', '支付方式', '配送方式', '自提门店', '联系人', '联系手机', '提货码', '核销状态', '核销员', '核销时间'];
        $exportExt = 'xls';
        $exportData = [];
        foreach ($lists as $item){
            ...
            $exportData[] = [$orderSn, $item['order_type_text'], $item['create_time'], $item['pay_time'], $item['order_source_text'], $item['user_sn'], $item['user_nickname'], $level, $goodsStr,$item['total_num'], $item['shipping_price'], $item['goods_price'], $item['discount_amount'], $item['order_amount'], $item['pay_way_text'], $item['delivery_type'], $item['shop_name'], $item['consignee'], $item['mobile'], $item['pickup_code'], $item['verification_status_text'], $item['verifier_name'], $item['verification_time']];
        }
